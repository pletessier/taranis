...
RedisIndexIOWriter::RedisIndexIOWriter(cpp_redis::client* redis, const std::string &db_name, const std::string &index_name) {
    this->redis = redis;
    key = fmt::format("db/{}/idx/{}/index", db_name, index_name);
    this->redis->set(key, "", [](cpp_redis::reply &reply) {});
}

RedisIndexIOWriter::~RedisIndexIOWriter() = default;
